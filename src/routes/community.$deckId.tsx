  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background">
        <SiteHeader />
        <main className="mx-auto max-w-5xl px-6 py-16 text-center text-muted-foreground">Loading deck...</main>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <main className="mx-auto max-w-5xl px-6 py-16 text-center">
          ...
          <Button asChild className="mt-6"><Link to="/community">Back to Community</Link></Button>
        </main>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-5xl px-6 py-10">
        <Link to="/community" className="mb-8 inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
        ...
        <section className="rounded-3xl border border-border bg-card p-6 md:p-8">
          <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              <Button variant="outline" className="rounded-full" onClick={onLike}>
                <Heart className={`h-4 w-4 ${deck.liked ? "fill-current text-destructive" : ""}`} /> {deck.likes}
              </Button>
              <Button variant="outline" className="rounded-full" onClick={onSave}>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-5">
            <div className="rounded-2xl bg-background border border-border px-4 py-3"><p className="text-xs text-muted-foreground">Cards</p><p className="font-display text-2xl">{deck.cardCount}</p></div>
            <div className="rounded-2xl bg-background border border-border px-4 py-3"><p className="text-xs text-muted-foreground">Learners</p><p className="font-display text-2xl">{deck.totalLearners}</p></div>
            <div className="rounded-2xl bg-background border border-border px-4 py-3"><p className="text-xs text-muted-foreground">Rating</p><p className="font-display text-2xl">{deck.rating || "New"}</p></div>
            <div className="rounded-2xl bg-background border border-border px-4 py-3"><p className="text-xs text-muted-foreground">Views</p><p className="font-display text-2xl">{deck.views}</p></div>
            <div className="rounded-2xl bg-background border border-border px-4 py-3"><p className="text-xs text-muted-foreground">Copies</p><p className="font-display text-2xl">{deck.copies}</p></div>
          </div>

          <div className="mt-6 flex flex-wrap items-center gap-2">
            <span className="text-sm text-muted-foreground">Rate this deck:</span>
            {[1, 2, 3, 4, 5].map((value) => (
              <button key={value} onClick={() => onRate(value)} className="rounded-full p-1 text-primary hover:bg-secondary" aria-label={`Rate ${value}`}>
        ...
        <section className="mt-8 grid gap-6 lg:grid-cols-[1fr_320px]">
          <div className="rounded-3xl border border-border bg-card p-6">
            ...
            <div className="mt-4 space-y-2">
              {cards.map((card) => (
                <div key={card.id} className="rounded-2xl border border-border bg-background px-4 py-3">
          ...
          <aside className="rounded-3xl border border-border bg-card p-6 h-fit">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Report inappropriate content for admin review.</p>
            <Input className="mt-4" placeholder="Reason" value={reportReason} onChange={(e) => setReportReason(e.target.value)} />
            <Button variant="outline" className="mt-3 w-full rounded-full" onClick={onReport} disabled={reportReason.trim().length < 3}>
