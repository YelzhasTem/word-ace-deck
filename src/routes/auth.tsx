...
function getAuthErrorMessage(err: unknown) {
  ...
  if (authError?.code === "weak_password" || lowerMsg.includes("weak")) {
    return "Пароль слишком простой. Используйте более надежный пароль.";
  }

  if (authError?.code === "invalid_credentials" || lowerMsg.includes("invalid login")) {
    return "Неверный email или пароль. Проверьте данные или создайте аккаунт.";
  }

  if (lowerMsg.includes("email logins are disabled")) {
    return "Вход по email отключен в Supabase. Включите Email provider в Authentication -> Providers.";
  }
  ...
  if (authError?.code === "email_not_confirmed" || lowerMsg.includes("not confirmed")) {
    return "Email еще не подтвержден. Проверьте почту и перейдите по ссылке подтверждения.";
  }

  if (authError?.status === 400) {
    return "Supabase отклонил запрос. Проверьте данные и настройки провайдеров Auth.";
  }

  return msg || "Не удалось выполнить действие. Проверьте настройки Supabase Auth.";
}
      ...
      <div className="w-full max-w-md rounded-2xl border border-border bg-card p-8 shadow-sm">
        ...
        {session ? (
          <div className="mt-6 space-y-5">
            <div>
              <h1 className="text-2xl font-bold tracking-tight">Вы уже вошли</h1>
              <p className="mt-2 text-sm text-muted-foreground">
                Аккаунт: <span className="font-medium text-foreground">{session.user.email}</span>
              </p>
            </div>
          ...
          <>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              {mode === "login"
                ? "Войдите по email и паролю или через Google."
                : "Создайте аккаунт Supabase Auth, чтобы сохранять прогресс."}
            </p>
