        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground max-w-xl">
              Включите режим, чтобы переводить слова из короткой памяти в долгую через возрастающие интервалы.
            </p>
          </div>
      ...
      {summary.ready > 0 ? (
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-2xl bg-accent/10 px-4 py-3">
          <p className="text-sm flex items-center gap-2">
            ...
            {summary.ready} {summary.ready === 1 ? "слово готово" : "слов готовы"} к припоминанию
            {readyDecks.length > 1 ? ` · ${readyDecks.length} колод` : ""}
          </p>
          {firstReadyDeck && (
        ...
        <p className="text-sm text-muted-foreground">
          Следующий повтор {formatDueIn(summary.nextDue)}. Заглядывайте позже — мы напомним.
        </p>
      ) : (
        <p className="text-sm text-muted-foreground">
          Запланированных слов пока нет. Добавьте карточки — они автоматически встанут в очередь.
        </p>
      )}
    </section>
  );
