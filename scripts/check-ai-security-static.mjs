...
assert.match(urlSource, /URL_FETCH_MAX_REDIRECTS = 3/);
assert.match(urlSource, /URL_FETCH_TIMEOUT_MS = 10_000/);
assert.match(urlSource, /url\.protocol !== \"https:\"/);
assert.match(urlSource, /visitedUrls/);
assert.match(urlSource, /controller\.signal/);
assert.doesNotMatch(urlSource, /redirect:\s*["']follow["']/);
assert.doesNotMatch(aiSource, /Source: \$\{data\.url\}/);
