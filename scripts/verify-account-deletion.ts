...
import { cleanupAccountStorage } from "../src/lib/account-deletion-workflow.ts";
...
try {
  ...
  const { otherDeckId } = await seedUserData(userA.id, userB.id, clientA);
  ...
  const blockedProfile = await clientA
    .from("profiles")
    .update({ display_name: "Must not save" })
    .eq("user_id", userA.id);
  assert.ok(blockedProfile.error);
  const blockedDeck = await clientA.rpc("create_deck_with_cards", {
    p_name: "Must not exist",
    p_description: "",
    p_cover_color: null,
    p_target_language: "en",
    p_definition_language: "ru",
    p_cards: [{ term: "blocked", definition: "blocked", position: 0 }],
    p_collection_id: null,
    p_use_default_collection: false,
    p_idempotency_key: randomUUID(),
  });
  assert.ok(blockedDeck.error);
  const blockedAvatar = await clientA.storage
    .from("avatars")
    .upload(`${userA.id}/avatar-after-request.png`, new Uint8Array([137, 80, 78, 71]), {
      contentType: "image/png",
    });
  assert.ok(blockedAvatar.error);
  ...
  if (finalized.error || finalized.data?.[0]?.job_status !== "completed") {
    throw new Error("Could not finalize account deletion");
  }
  ...
  const staleProfileMutation = await clientA.from("profiles").insert({
    user_id: userA.id,
    username: `stale_${userA.id.slice(0, 8)}`,
    display_name: "Must not return",
  });
  assert.ok(staleProfileMutation.error);
  const staleAvatarUpload = await clientA.storage
    .from("avatars")
    .upload(`${userA.id}/avatar-from-stale-token.png`, new Uint8Array([137, 80, 78, 71]), {
      contentType: "image/png",
    });
  assert.ok(staleAvatarUpload.error);
  ...
  assert.equal(await exactCount("decks", "id", otherDeckId), 1);
